...
function setLanguage(lang) {
    ...
    // 1. חילוץ וצריבה מיידית של העוגיות עבור מנוע הפייתון ב-main.py
    ...
    // 2. הניקוי האוניברסלי של ה-Cache: מוחקים את כל זיכרון השפה הישן של הדפדפן כדי לנקות את 18 הדפים!
    sessionStorage.clear();

    // 3. טעינה קוסמטית של קובץ ה-JSON הסטטי על אלמנטים במסך
    loadLanguage(lang);

    if (typeof updateAllPrices === "function") updateAllPrices();

    // כפיית סינכרון אקטיבי מול השרת ברגע בחירת השפה בדף הלוגין 
    // מזהה בצורה חלקה אם המשתמש נמצא כרגע פיזית בדף הכניסה (Login)
    if (window.location.pathname === "/login" || window.location.pathname === "/" || document.getElementById("auth-form")) {
        
        // שליפת הערך הקיים של השפה מתוך העוגיות בדפדפן כדי למנוע לולאות ריענון אינסופיות
        const cookiePair = document.cookie.split('; ').find(row => row.trim().startsWith('lang='));
        
        //  ניקוי קשיח של רווחים ונקודה-פסיק מהעוגיה למניעת לולאות אינסופיות!
        let currentCookieVal = cookiePair ? cookiePair.split('=')[1] : '';
        if (currentCookieVal) {
            currentCookieVal = currentCookieVal.trim().replace(';', '');
        }
        
        //  מבצעים הפניה לראוט השרת הרשמי שלך רק אם השפה שנבחרה שונה מהעוגייה הנוכחית של השרת!
...
function loadLanguage(lang) {
    fetch(`/static/${lang}.json`)
        ...
        .then(data => {
            ...
            // הזנקה חוזרת של בניית ימי השעון לאחר טעינת השפה בצורה מאובטחת
