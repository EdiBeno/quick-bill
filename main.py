...
def initialize_database():
    with app.app_context():
        ...
        # >>> תיקון הזהב המובנה להתקנות חדשות בענן (Postgres Sequence Sync) <<<
        try:
            db.session.execute(db.text(
                "SELECT setval('company_id_seq', COALESCE((SELECT MAX(id)+1 FROM company), 2), false);"
            ))
        except Exception:
            # הגנה: אם מריצים בלוקאל על SQLite, הפקודה תיכשל בשקט ללא שום שגיאה
            pass
...
def customer_dashboard_view():
    try:
        ...
        # שליחת כל הלקוחות לדף החשבונית (עם local_id!)
