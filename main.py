...
def clients():
    ...
    # ==========================================
    # 🛠️ LAYER A: HANDLE COMBOBOX UPDATE (POST)
    # ==========================================
    if request.method == 'POST':
        try:
            target_user_id = request.form.get('user_id')
            new_status = request.form.get('status') # Expecting 'active' or 'blocked'

            if target_user_id and target_user_id.isdigit():
                uid = int(target_user_id)
                user_to_update = db.session.get(User, uid)
                
                if user_to_update:
                    # Enforce multi-tenant authorization security boundaries
            ...
            import traceback
            traceback.print_exc()
            print(f"❌ Error updating combobox status: {e}")
