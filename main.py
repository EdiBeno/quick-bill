...
def base_invoice_context(customer_id=None):
    ...
    for p in Product.query.filter_by(company_id=company_id).all():
        item_file = load_item_file(company_id, p.local_id if p.local_id else p.id) or {}
        translated_core = load_item_translated(p, language, company_id)

        p_name = translated_core.get("name") or p.name or ""
        p_desc = translated_core.get("description") or p.description or ""
        i_cat = translated_core.get("income_category") or p.income_category or "service"
    ...
    # detect self-invoice correctly
    if customer_id is not None:
        # SELF-INVOICE → company is the customer
        if str(customer_id) == "0":
            ...
            if c_obj:
                ...
                customer_data = {
                    ...
                    "address": trans.get("address", c_obj.address or ""),
                    "city": trans.get("city", c_obj.city or ""),
                    "postal_code": c_obj.postal_code or "",
                    "id_number": c_obj.id_number or "",
                    "phone": c_obj.phone or "",
                    "email": c_obj.email or "",
                    "message": trans.get("message", c_obj.message or "")
                }
...
def invoice_context(invoice_id=None):
    try:
        ...
        # נשתמש בלקוח מתוך base_ctx, כדי ש-SELF-INVOICE יעבוד תמיד
...
def save_invoice():
    ...
    # SELF‑INVOICE MODE
    if customer_id is None or str(customer_id) == "0":
...
def customer():
    ...
    if request.method == 'POST':
        ...
        # ------------------ CREATE NEW CUSTOMER  ------------------
            ...
            try:
                # SAFE PLACEHOLDER: מונע קריסות בגלל ה-10 שניות עיכוב של התרגום
...
def api_get_customer(customer_id):
    try:
        ...
        # קריאה הרמטית למילון החדש – אם קובץ ה-JSON ב-10 שניות עיכוב של תרגום ברקע, השורה הזו תחזיר מיד את נתוני ה-DB כגיבוי!
        trans = load_customer_translated(c, language, company_id=active_company_id) or {}
