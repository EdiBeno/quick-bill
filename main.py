...
def save_invoice():
    ...
    # === התיקון המדויק לשני סוגי הלקוחות: לקוח רגיל מול לקוח חשבונית עצמית ===
    checked_customer = None

    if str(customer_id) == "0":
        # מקרה 1: חשבונית עצמית (דיבידנד, שכירות, מניות) - שולף את לקוח חברת הבעלים של המשתמש
        owner_customer = Customer.query.filter_by(
            id_number=str(current_user.id),
            company_id=OWNER_COMPANY_ID
        ).first()

        if not owner_customer and current_user.email:
            owner_customer = Customer.query.filter_by(
                email=current_user.email,
                company_id=OWNER_COMPANY_ID
            ).first()
        ...
        if owner_customer:
            # מעדכן את ה-ID מ-"0" למזהה האמיתי של הלקוח בדאטהבייס
            customer_id = owner_customer.id
            checked_customer = owner_customer
        else:
            # מניעת קריסה אם הלקוח של חברת הבעלים לא נוצר עדיין
            checked_customer = True 
    else:
        # מקרה 2: לקוח חיצוני רגיל המשויך לחברה הפעילה הנוכחית
    ...
    if not checked_customer:
        ...
        # פתרון חסין אש לקריסות ב-Render - חוזר לדף הקודם בלי להסתמך על ראוטים שלא קיימים
        return redirect(request.referrer or '/')
...
def add_transaction():
    try:
        ...
        # פיקס מנצח ל-Render: אם מדובר בלקוח "0" או ריק, שומרים כ-None כדי שהדאטהבייס לא יקרוס
        if not raw_customer_id or str(raw_customer_id).strip() == "0" or raw_customer_id == "":
            final_customer_id = None
        else:
            final_customer_id = int(raw_customer_id) if str(raw_customer_id).isdigit() else None
