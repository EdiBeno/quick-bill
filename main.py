...
def load_item_file(company_id_or_obj, local_id=None, company_id=None):
    """טוען ומחזיר את קובץ ה-JSON הראשי של המוצר מהדיסק בצורה בטוחה"""
...
def save_inventory_transaction(
    ...
    """📝 יוצרת קובץ טרנזקציית מלאי פנימי בנפרד בדיסק לכל מנת אספקה (Batch)"""
    folder = ensure_product_folder(company_id, local_id)
    os.makedirs(folder, exist_ok=True)
    ...
    **kwargs
    ...
    thread = threading.Thread(
        ...
        args=(
            ...
            0,
            current_supplier_id,                 # הזרקת ה-supplier_id האמיתי מה-SQL
            supplier_name or "מלאי פתיחה / כללי", # הזרקת ה-supplier_name מהטופס
...
def base_invoice_context(customer_id=None):
    ...
    customer_data = None
    if customer_id:
        # שליפת הלקוח המשויך לחשבונית מהחברה הנוכחית
        c_obj = Customer.query.filter_by(id=customer_id, company_id=company_id).first()
        
        # תרגום חזרה ל-Frontend: אם הלקוח הוא רשומת הבעלים/החברה הפנימית
        if c_obj and (c_obj.id_number == str(current_user.id) or getattr(c_obj, 'customer_name', '').startswith("★")):
            if company_obj:
                customer_data = {
                    "id": "0",  # מזריק "0" בחזרה ל-JS כדי שתיבת הבחירה תזהה את החברה עצמה!
...
def invoice_context(invoice_id=None):
    try:
        ...
        # חדירה למפתח האב cancellation_reason מתוך הנתיב המבודד בדיסק לפי השפה האקטיבית!
        cancellation_reason_trans = ""
...
def save_invoice():
    ...
    # === תיקון מוחלט לחשבוניות פנימיות: העתקת נתוני החברה בצד ימין אל פרטי הלקוח ===
    if str(customer_id) == "0":
        # 1. שליפת אובייקט החברה האקטיבית כדי לקחת ממנו את הנתונים האמיתיים
        from app.models import Company  # ודא שהאימפורט נכון אצלך
        company_obj = Company.query.get(company_id)

        # 2. חיפוש הלקוח הפנימי של החברה הנוכחית
        owner_customer = Customer.query.filter_by(
            company_id=company_id,
            id_number=str(company_obj.company_id_number if company_obj else current_user.id)
        ).first()

        # 3. אם לא קיים או שצריך לעדכן את הפרטים שלו שיהיו זהים לחברה האקטיבית
        ...
        # 4. שורת הזהב: הזרקת הנתונים האמיתיים של החברה האקטיבית לתוך הלקוח!
        if company_obj:
            owner_customer.customer_name = company_obj.name  # יהפוך ל"חתולים בע"מ" במקום חברה #8!
            owner_customer.id_number = company_obj.company_id_number or ""
            owner_customer.address = company_obj.address or ""
            owner_customer.city = company_obj.city or ""
            owner_customer.postal_code = company_obj.postal_code or ""
            owner_customer.phone = company_obj.phone or ""
            owner_customer.email = company_obj.email or ""
        
        db.session.flush()

        # מעדכנים את מזהה הלקוח ל-ID של רשומת החברה המשוכפלת
        customer_id = owner_customer.id
        checked_customer = True
            
    else:
        # טיפול רגיל בלקוח חיצוני
    ...
    if invoice_id:
        ...
        # שלב ה': עדכון או יצירת תנועה פיננסית ביומן התנועות (Transaction)
        ...
        if existing_trans:
            ...
            existing_trans.customer_id = customer_id
    ...
    # ====== 2. יצירת חשבונית חדשה לחלוטין (מצב יצירה) ======
        ...
        new_invoice = Invoice(
            ...
            customer_id=customer_id,  # This now contains the real database owner ID instead of "0"!
            sub_total=sub_total,
            vat_amount=vat_amount,
            grand_total=grand_total,
            vat_rate=vat_rate,
        ...
        # We enforce that the master variable "invoice" references our freshly saved database record.
        invoice = new_invoice
        ...
        for item_json in items:
            ...
            # Clean and sanitize the text representation string
            p_id_str = str(p_id_val).strip().lower()

            if p_id_str in ['rent', 'stocks', 'dividend', 'unspecified']:
                db.session.add(InvoiceItem(
                    invoice_id=new_invoice.id,  
                    product_id=p_id_str,        
                    description=item_description or p_id_str, 
                    quantity=qty,
                    unit_price=u_price,
                    discount=disc,
                    total_price=row_total,
                    cost_price_at_time=0.0,
                    income_category=p_id_str
                ))
                continue

            # Safety check: If it's not a digit, it cannot be a catalog sequence integer lookup.
            if not str(p_id_val).strip().isdigit():
                continue

            prod = Product.query.filter_by(id=int(p_id_val), company_id=company_id).first()
            if not prod:
            ...
            db.session.add(InvoiceItem(
                invoice_id=new_invoice.id,
                product_id=int(db_product_id) if db_product_id is not None else None, 
                description=item_description, 
                quantity=qty,
                unit_price=u_price,
                discount=disc,
        ...
        for i in range(len(amounts)):
            ...
            db.session.add(Payment(
                company_id=company_id,
                invoice_id=new_invoice.id,  # Perfect, points to the flushed integer id
                payment_date=p_date if p_date else datetime.today().date(),
                payment_method=methods[i] if i < len(methods) else "",
                payment_amount=amt,
                bank=banks[i] if i < len(banks) else "",
...
def cancel_invoice(invoice_id):
    ...
    if trans:
        ...
        # יומן התנועות: מציג את התיאור והסיבה המדויקת לפי שפת הממשק האקטיבית
        if current_lang != "he":
...
def customer():
    ...
    # הזרקה יזומה של מפתחות הדיווח העצמי (0 ו-"0") למקרה קצה שהחברה מציגה את עצמה
    ...
    # לולאת הסנכרון המרכזית על כל המשתמשים (הרגילים והזמניים)
    for c in all_customers_combined:
        trans = load_customer_translated(c, language, company_id=active_company_id) or {}
        
        # תיקון מפתחות המילון: בודק גם customer_name וגם name כגיבוי
        c_name_translated = trans.get("customer_name") or trans.get("name") or c.customer_name or ""
        
        final_data = {
            "name": c_name_translated,
        ...
        # המפתח המקורי שלך (כדי לא לשבור לוגיקות ישנות בקבצים אחרים)
        old_key = str(c.local_id) if getattr(c, "local_id", None) else f"user_{c.id}"
        customer_i18n_list[old_key] = final_data
        
        # === שורת הזהב של התיקון: רישום המידע תחת המפתחות הישירים של ה-DOM ===
        customer_i18n_list[c.id] = final_data
        customer_i18n_list[str(c.id)] = final_data
        
        if c.customer_name:
            customer_i18n_list[c.customer_name] = final_data
            # גיבוי תחת השם המתורגם למניעת שגיאות שפה
            if c_name_translated != c.customer_name:
                customer_i18n_list[c_name_translated] = final_data
...
def api_get_customer(customer_id):
    try:
        ...
        # Pre-fetch company details once for internal invoice routing use cases
        company_obj = db.session.get(Company, active_company_id)
        company_translated = load_company_translated(company_obj, language) if company_obj else {}

        # ------------------ 1. DIRECT INITIAL SELF-BILLING HOOK ------------------
        if customer_id == 0:
            if company_obj:
                return jsonify({
                    "id": 0,          
        ...
        # Query using the company localized ID context index parameter
        ...
        # ------------------ 2. EXISTING INTERNAL SELF-BILLING HOOK ------------------
        # FIX: If the row represents an internal self-billing company customer, inject the company variables directly!
        if c and (getattr(c, 'role', '') == 'owner' or getattr(c, 'customer_name', '').startswith("★")):
            if company_obj:
                return jsonify({
                    "id": c.id,          
                    "local_id": c.local_id,    
        ...
        # ------------------ 4. REGULAR REAL EXTERNAL CUSTOMER ------------------
        ...
        # Safe dictionary attribute extraction with reliable standard field values fallback
        return jsonify({
            "id": c.id,               
            "local_id": c.local_id,   
            "customer_name": trans.get("customer_name") or trans.get("name") or c.customer_name or "",
...
def search_customer():
    try:
        ...
        # ------------------לקוחות רגילים מהדאטהבייס ------------------
        for c in all_customers:
            # Fetch translations with strict validation fallbacks
            trans = load_customer_translated(c, language, company_id=active_company_id) or {}
            
            # Extract standard naming keys cleanly
            c_name_translated = trans.get("customer_name") or trans.get("name") or c.customer_name or ""
            ...
            # FIX 1: Store under the database primary key `id` AND `local_id` to eliminate front-end lookup conflicts!
            customer_i18n_list[c.id] = payload
            if c.local_id is not None:
                customer_i18n_list[c.local_id] = payload
                
            # FIX 2: Bind text labels to prevent search script conflicts
            if c.customer_name:
                customer_i18n_list[c.customer_name] = payload
                # Also store under the translated name string variant just in case
                if c_name_translated != c.customer_name:
                    customer_i18n_list[c_name_translated] = payload
