...
# -----------------------------------------------------------
#   משימות רקע - CELERY BACKGROUND TASKS
# -----------------------------------------------------------
    ...
    try:
        with app.app_context():
            ...
            if not os.path.exists(root_path):
                print(f"⚠️ Error: Source file not found at {root_path}")
        ...
        print(f"⚠️ Company translation task failed, retrying... Error: {exc}")
        raise self.retry(exc=exc, countdown=10)


# -----------------------------------------------------------
#   Company Translations - File Management & Routes
# -----------------------------------------------------------
...
# -----------------------------------------------------------
#  Customer Translation Background Tasks (Celery Core Engine)
# -----------------------------------------------------------
...
def load_customer_file(customer_id):
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not read customer JSON payload data file: {e}")
        return None
...
# -----------------------------------------------------------
#  Supplier Translation Background Tasks (Celery Core Engine)
# -----------------------------------------------------------
...
def load_supplier_file(supplier_id):
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not read supplier JSON payload data file: {e}")
        return None
...
# -----------------------------------------------------------
#  Product Translation Background Tasks (Celery Core Engine)
# -----------------------------------------------------------

@celery_app.task(name="app.product_translation_task", bind=True, max_retries=3)
def product_translation_task(
    self, 
    product_id, 
    name, 
    description, 
    ...
    try:
        with app.app_context():
            ...
            return f"✅ Success: Translated Product Profile ID: {product_id}"
...
def load_item_file(product_id):
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not read item JSON payload data file: {e}")
        return None



# -----------------------------------------------------------
#  Transaction Translation Background Tasks (Celery Core Engine)
# -----------------------------------------------------------

@celery_app.task(name="app.transaction_translation_task", bind=True, max_retries=3)
def transaction_translation_task(
    self, 
    transaction_id, 
    description, 
    amount, 
    ...
    try:
        with app.app_context():
            ...
            return f"✅ Success: Translated Transaction Profile ID: {transaction_id}"
...
def load_transaction_file(transaction_id):
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not read transaction JSON payload data file: {e}")
        return None


# -----------------------------------------------------------
#  Category Translation Background Tasks (Celery Core Engine)
# -----------------------------------------------------------
...
# -----------------------------------------------------------
#  Categories Helper - Save & Load JSON Translations
# -----------------------------------------------------------
...
def save_category_file(cat_id, name_trans):
    ...
    data = {
        "id": int(cat_id),
        "name": name_trans  # מילון של 30 שפות: {"he": "...", "en": "..."}
    }
...
def load_category_file(cat_id):
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not read category JSON data file: {e}")
        return None
    ...
    try:
        for cat_id in os.listdir(CATEGORIES_DIR):
            ...
            if data and "name" in data:
                ...
                categories_dict[str(cat_id)] = name
    except Exception as e:
        print(f"⚠️ Warning: Error aggregating total business classification categories: {e}")
    ...
    for p in [root_path, dir_path]:
        if os.path.exists(p):
            try:
                ...
                print(f"⚠️ Warning: Could not remove file {p}: {e}")
...
def invoice_context(invoice_id=None):
    try:
        ...
        # ----- All Products (INTEGRATED & SAFELY ISOLATED) -----
        products_json = []
        for p in Product.query.all():
            ...
            products_json.append({
                "id": p.id,
                "name": p_name,   #  Passed down safely as a flat localized string
                "price": float(p.price or 0),
                "cost_price": float(p.cost_price or 0),
                "quantity": int(p.quantity or 0),
                "income_category": i_cat  
    ...
    try:
        ...
        with app.app_context():
            ...
            if not invoice or not invoice.customer or not invoice.customer.email:
                return f"❌ Aborted: Missing entity or email data block for Invoice ID {invoice_id}"
...
def manage_products():
    ...
    if request.method == 'POST':
        ...
        try:
            # Convert from HTML (YYYY-MM-DD) to DB (DD/MM/YYYY)
    ...
    if search:
        ...
        for p in all_products:
            ...
            if match: filtered_objects.append(p)
...
def products_list():
    ...
    for p in products:
        ...
        if item_file:
            names_dict = item_file.get("name") or {"he": name_he}
            descs_dict = item_file.get("description") or {"he": desc_he}
        else:
...
def supplier():
    ...
    # Build comprehensive mapping dictionary of supplier rows translated for the selection grids
    supplier_i18n_list = {
    ...
    return render_template(
        ...
        supplier_i18n=supplier_i18n,          # Serves clean localized data fields for edit form context
        supplier_i18n_list=supplier_i18n_list, # Serves translated rows for listing views
        today=today_str,
...
def add_transaction():
    try:        
        ...
        if trans_type == 'income':
            ...
            if p_id:
                ...
                if product:
                    ...
                    # Secure historical acquisition prices exclusively for inventory merchandise profiles
        ...
        transaction_translation_task.delay(
