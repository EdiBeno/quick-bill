...
def save_invoice():
    ...
    # === תיקון מוחלט לחשבוניות פנימיות (שכירות, דיבידנד, מניות) ===
    if str(customer_id) == "0":
        # נחפש לקוח פנימי שרשום ספציפית תחת החברה הנוכחית ומייצג את החברה/בעלים
        owner_customer = Customer.query.filter_by(
            company_id=company_id,
            role='customer',  # או 'owner' אם שינית את השדה הזה במודל
            id_number=str(current_user.id)
        ).first()

        # אם עדיין לא קיים לקוח כזה לחברה החדשה הזו, ניצור אותו בצורה מאובטחת ומבודדת
        if not owner_customer:
            ...
            owner_customer = Customer(
                local_id=next_local_id,
                company_id=company_id,
                customer_name=f"★ חשבונית פנימית - חברה #{company_id}",
                id_number=str(current_user.id),
                role='customer',
                is_active=True
            )
            db.session.add(owner_customer)
            db.session.flush()  # פוסטגרס מייצר מזהה ייחודי חוקי

        # מעדכנים את מזהה הלקוח ל-ID האמיתי של רשומת החברה הפנימית
