...
def save_invoice():
    ...
    if str(customer_id) == "0":
        # נשלוף את הלקוח המנהל שנוצר בשלב פתיחת החברה תחת חברת הבעלים הגלובלית
        ...
        if owner_customer:
            # מצאנו אותו! מחליפים את ה-"0" ב-ID הפוסטגרסי החוקי שלו
            customer_id = owner_customer.id
            checked_customer = True
        else:
            # הגנה למקרה קצה שהשורה לא נמצאה בכלל
            flash("שגיאה: לא נמצאו נתוני מנהל מערכת תואמים", "error")
            return redirect(url_for('invoice'))
    ...
    # ====== 2. יצירת חשבונית חדשה לחלוטין (מצב יצירה) ======
        ...
        for item_json in items:
            ...
            # 1. Safe cleaning of p_id_val to avoid extra spaces or case mismatches
            p_id_str = str(p_id_val).strip().lower()

            if p_id_str in ['rent', 'stocks', 'dividend', 'unspecified']:
                db.session.add(InvoiceItem(
                    invoice_id=new_invoice.id,  # references your correct flushed ID
                    product_id=None,            # FIX: product_id column in database is an integer, so we don't save a string here!
