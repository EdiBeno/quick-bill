...
def save_invoice():
    ...
    # הגדרת משתנים מראש למניעת שגיאות UnboundLocalError בכל מצבי הריצה
    db_customer_id = None
    ...
    if str(customer_id).strip() == "0" or not customer_id:
        ...
        if company_obj:
            # 1. שליפת הלקוח הפנימי של החברה לפי הנתונים הדינמיים של החברה הפעילה הנוכחית!
            owner_customer = Customer.query.filter_by(
                company_id=company_id
            ).filter(
                (Customer.email == company_obj.email) | 
                (Customer.id_number == str(company_obj.company_id_number))
            ).first()

            # 2. רשת ביטחון: אם המשתמש מעולם לא שמר את עמוד החברה, המערכת בוראת את לקוח המנהל אוטומטית במקום!
            if not owner_customer:
                try:
                    last_cust = Customer.query.filter_by(company_id=company_id).order_by(Customer.local_id.desc()).first()
                    next_local_id = 1 if not last_cust else (last_cust.local_id or 0) + 1

                    owner_customer = Customer(
                        local_id=next_local_id,
                        company_id=company_id,
                        customer_name=company_obj.name,
                        address=company_obj.address,
                        city=company_obj.city,
                        phone=company_obj.phone,
                        email=company_obj.email,
                        id_number=str(company_obj.company_id_number or ""),
                        date=datetime.today().strftime('%d/%m/%Y'),
                        role='customer',
                        is_active=True
                    )
                    db.session.add(owner_customer)
                    db.session.flush()  # פוסטגרס מייצר כאן ID אוטומטי חוקי לחברה הספציפית הזו!
                except Exception:
                    db.session.rollback()
                    # גיבוי אחרון בהחלט
        ...
        if owner_customer:
            db_customer_id = owner_customer.id  # ה-ID האוטומטי החוקי שפוסטגרס מכיר לחברה הפעילה!
