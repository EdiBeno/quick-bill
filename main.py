...
def load_item_translated(product, language):
    ...
    # אם אין קובץ עדיין → מחזיר מה‑DB
    if not data:
        return {
            "name": product.name or "",
            "description": product.description or "",
            "price": product.price or 0.0,
            "income_category": product.income_category or "service",
            "cost_price": product.cost_price or 0.0,
            "stock_in": product.stock_in or 0,
            "stock_out": product.stock_out or 0,
            "supplier_id": product.supplier_id or None,
            "received_date": product.received_date or None
        }
    ...
    return {
        "name": get_val("name", product.name),
        "description": get_val("description", product.description),
        "price": data.get("price", product.price),
        "income_category": data.get("income_category", product.income_category),
        "cost_price": data.get("cost_price", product.cost_price),
        "stock_in": data.get("stock_in", product.stock_in),
        "stock_out": data.get("stock_out", product.stock_out),
        "supplier_id": data.get("supplier_id", product.supplier_id),
        "received_date": data.get("received_date", product.received_date)
    }
