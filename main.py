...
def customer_dashboard_view():
    try:
        ...
        #  משוך לפי מספר החשבונית המקומי והרץ של החברה למניעת סלטים במע"מ!
...
def base_invoice_context(customer_id=None):
    ...
    if customer_id is not None:
        # === אבחנה 1: חשבונית עצמית של החברה ===
        ...
        # === אבחנה 2: חשבונית לקוח רגילה מהדאטאבייס ===
        else:
...
def save_invoice():
    ...
    # === הגשר החכם למניעת קריסות ב-Render ===
    # הממשק שולח "0" עבור חשבונית עצמית. פוסטגרס דורש ID מספרי של לקוח אמיתי.
    ...
    if invoice_id:
        ...
        #  שומר הסף האטומי: חוסם לחלוטין שמירה או עריכה מחדש של מסמך שכבר בוטל!
        ...
        # שלב ב': עדכון נתוני כותרת החשבונית (שימוש ב-db_customer_id המספרי התקין)
        ...
        # שלב ה': עדכון או יצירת תנועה פיננסית ביומן התנועות (שימוש ב-db_customer_id המספרי התקין)
...
# ==================== מיפוי שמות הפריטים עבור הטבלאות בדף התצוגה HTML ====================
        ...
        for customer in all_customers:
            for inv in customer.invoices:
                ...
                for item in inv.items:
                    prod_obj = None
                    # חומת אש אטומית: וידוא שהערך מספרי בלבד לפני פנייה לפוסטגרס למניעת קריסות טיפוסים ב-Render
                    if item.product_id and str(item.product_id).isdigit():
                        prod_obj = Product.query.filter_by(local_id=int(item.product_id), company_id=active_company_id).first()
                        if not prod_obj:
                            prod_obj = Product.query.filter_by(id=int(item.product_id), company_id=active_company_id).first()
