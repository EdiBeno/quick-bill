...
def base_invoice_context(customer_id=None):
    ...
    if customer_id:
        ...
        if c_obj and getattr(c_obj, 'role', '') == 'owner':
            if company_obj:
                customer_data = {
                    "id": "0",  
...
def save_invoice():
    ...
    # === התיקון המדויק עבור חשבונית פנימית (דיבידנד, שכירות, מניות) ===
    if str(customer_id) == "0":
        # 1. קודם כל נחפש לקוח רשום ששייך ספציפית לחברה האקטיבית הנוכחית ומייצג אותה פנימית
        owner_customer = Customer.query.filter_by(
            company_id=company_id,
            id_number=str(current_user.id)
        ).first()

        # 2. אם לא מצאנו, נבצע את החיפוש המקורי שלך תחת חברת הבעלים הגלובלית, אך נוודא שהוא ייחודי למשתמש המחובר בלבד
        ...
        # 3. הגנה מוחלטת: אם הרשומות הושמדו או לא נמצאו, ניצור פרופיל פנימי ייחודי לחברה הזו כדי למנוע קריסה או ערבוב נתונים
        if not owner_customer:
        ...
        # מצאנו או יצרנו רשומה חוקית ומבודדת! מחליפים את ה-"0" ב-ID הפוסטגרסי החוקי שלו
        customer_id = owner_customer.id
        checked_customer = True
            
    else:
        # טיפול בלקוח חיצוני רגיל
