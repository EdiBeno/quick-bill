...
from flask import Flask, render_template, request, redirect, url_for, session, jsonify, make_response, Response, flash, current_app
...
def generate_translations(text, source_lang="auto"):
    ...
    def translate_single(lang):
        try:
            ...
            if translated and not any(err in str(translated) for err in ["Error", "error", "500", "html", "That’s an error"]):
...
def login():
    try:
        ...
        if login_email == OWNER_USERNAME and password == OWNER_PASSWORD:
            ...
            owner_user = User.query.filter_by(email=OWNER_USERNAME, company_id=OWNER_COMPANY_ID).first()
            if not owner_user:
                flash("בעל המערכת לא מוגדר כמשתמש בבסיס הנתונים", "danger")
                return redirect(url_for('login'))

            login_user(owner_user, force=True)

            session['owner_access']  = True
            session['user_id']       = owner_user.id
            session['role']          = owner_user.role
            session['user_role']     = owner_user.role
            session['company_id']    = owner_user.company_id
            session['company_name']  = "לרקוד על הגג"
            session['company_email'] = owner_user.email
            session['customer_id']   = None

            owner_user.last_login = datetime.utcnow()
            db.session.commit()
            db.session.close()
    ...
    try:
        ...
        if role == 'manager':
            ...
            try:
                ...
                # 3. הפעלת פונקציות הרקע עם הכתובת והעיר האמיתיות כדי שהתרגום יווצר באנגלית מושלמת!
                translate_company_in_background(
                    new_company.id, 
                    new_company.name, 
                    getattr(new_company, 'company_id_number', '') or "", 
                    getattr(new_company, 'deduction_file', '') or "", 
                    getattr(new_company, 'address', '') or "", 
                    getattr(new_company, 'city', '') or "", 
                    getattr(new_company, 'postal_code', '') or "", 
                    user.email, 
                    ""
                )
                ...
                translate_customer_in_background(
                    ...
                    getattr(platform_customer, 'address', '') or "", 
                    getattr(platform_customer, 'city', '') or "", 
                    ""
                )
            except Exception as e:
        ...
        # ------------------ 2) CUSTOMER – לקוח של חברה קיימת ------------------
            ...
            try:
                # יצירת קובץ Placeholder זמני ללקוח החדש בנתיב המדויק שלו! `customers/{company_id}_{local_id}/customer.json`
    ...
    try:
        ...
        if is_owner:
            ...
            for c in owner_customers:
                ...
                if c.local_id:
                    cust_folder = ensure_customer_folder(OWNER_COMPANY_ID, c.local_id)
                    if cust_folder and os.path.exists(cust_folder):
                        cust_file = os.path.join(cust_folder, "customer.json")
                            ...
                            try:
                                with open(cust_file, "r", encoding="utf-8") as f:
                                    ...
                                    if isinstance(name_dict, dict):
                                        display_name = name_dict.get(lookup_lang) or name_dict.get("he") or display_name
                            except Exception as e:
                                print(f"⚠️ Error parsing client JSON on display sync: {e}")
            ...
            for c in company_records:
                ...
                #   Pull translated Manager's customer name directly from customer.json!
                if c.local_id:
                    cust_folder = ensure_customer_folder(active_company_id, c.local_id)
                    if cust_folder and os.path.exists(cust_folder):
                        cust_file = os.path.join(cust_folder, "customer.json")
                        if os.path.isfile(cust_file):
                            try:
                                ...
                                print(f"⚠️ Error parsing manager's client JSON on layout: {e}")
            ...
            for emp_user in company_employees_users:
                ...
                display_name = emp_profile.employee_name if emp_profile else (emp_user.username or emp_user.email or "עובד חדש")
        ...
        #  MULTI-TENANT TRANSLATION PAYLOAD LAYER - NO CLASH KEY LOCKS
        if is_owner:
        ...
        for cust in all_customers_db:
            ...
            #   Created airtight dictionary keys combining company_id + local_id to stop name clashing!
...
def update_access():
    try:
        ...
        try:
            ...
            # CUSTOMER JSON SYNC + TRANSLATION
                ...
                cust_address = getattr(customer, 'address', '') or ""
                cust_city    = getattr(customer, 'city', '') or ""
                cust_message = getattr(customer, 'message', '') or ""
    ...
    try:
        ...
        if old_role != new_role:
            ...
            if new_role == 'manager':
                ...
                # מחיקת רשומות לקוח/עובד ישנות של המשתמש בחברה הישנה
                ...
                #  אין יצירת Customer לבעלים / מנהל בפלטפורמה OWNER_COMPANY_ID
                ...
                try:
                    ...
                    if active_comp:
                        translate_company_in_background(
                            active_comp.id,
                            active_comp.name,
                            getattr(active_comp, 'company_id_number', '') or "",
                            getattr(active_comp, 'deduction_file', '') or "",
                            active_comp.address,
                            active_comp.city,
                            getattr(active_comp, 'postal_code', '') or "",
                            user.email,
                            ""
                        )
                    ...
                    print(f"⚠️ Background translation failed in customer conversion: {bg_err}")
    ...
    try:
        ...
        if is_self_billing:
            ...
            customer_json = {
                ...
                "customer_name": customer.customer_name or customer.email,
        ...
        # שליפת מטבע גיבוי מתוך העוגיות במידה ואין עדיין חשבוניות בדף
        active_currency_fallback = request.cookies.get('currency', 'ILS')
        ...
        base_ctx.update({
            ...
            "invoice_currency": current_active_inv.currency if current_active_inv else active_currency_fallback,
...
def employee_dashboard():
    try:
        ...
        # מנהל לא יכול לראות עובד מחברה אחרת
        ...
        # מצב שעון נוכחי
        shift_state = ShiftState.query.filter_by(
        ...
        for shift in my_shifts:
            if shift.startTime:
                st_val = str(shift.startTime)
                shift.startTime = st_val.split("T")[-1][:5] if "T" in st_val else st_val[:5]

            if shift.endTime:
                et_val = str(shift.endTime)
                shift.endTime = et_val.split("T")[-1][:5] if "T" in et_val else et_val[:5]

        # טעינת חברה מתורגמת דרך ה־helper
        company_obj = db.session.get(Company, active_company_id)
...
def run_company_translation(
    ...
    try:
        import time  # ויסות קצב הבקשות מול גוגל

        # תרגום שם החברה
        name_trans = generate_translations(name or "")
        time.sleep(0.4)  # השהיה קלה ברקע למניעת חסימת הצפה (Rate Limit) של ה-API של גוגל
...
def translate_customer_in_background(customer_id, company_id, name, address, city, message):
    # --- חומת אש: אם המזהה הוא 0 או ריק, עצור מיד ואל תהרוס את גוגל! ---
    if not customer_id or str(customer_id) == "0" or customer_id == 0:
        print(f"✔ Safeguard triggered: Blocked translation request for customer ID 0 to protect Google API.")
        return
...
def run_customer_translation(company_id, local_id, name, address, city, message):
    try:
        ...
        # תרגום שם הלקוח
        name_trans = generate_translations(name or "")
        time.sleep(0.4)  # השהיה קלה ברקע למניעת חסימת הצפה (Rate Limit)
...
def load_customer_translated(customer, language, company_id=None):
    ...
    # --- חומת אש: אם זה לקוח זמני או מזהה 0, תחזיר מיד את הגיבוי ואל תשלח כלום לגוגל! ---
    cust_id = getattr(customer, "id", None)
    if not customer or cust_id == 0 or str(cust_id) == "0":
        return fallback_data
...
def company():
    try:
        ...
        # -------- ענף חברה רגילה --------
            ...
            # -------- סנכרון רשומת Customer עבור בעל החברה ב־OWNER_COMPANY --------
            ...
            if not owner_customer_row:
                last_cust = Customer.query.filter_by(company_id=OWNER_COMPANY_ID)\
                    .order_by(Customer.local_id.desc()).first()
                next_local_id = 1 if not last_cust else (last_cust.local_id or 0) + 1

                # לא קובעים id ידנית — נותנים ל־Customer לקבל primary key אוטומטי
                ...
                db.session.flush()  # כדי שיהיה owner_customer_row.id

            # עדכון פרטי הלקוח לפי פרטי החברה
            ...
            # תרגום רשומת הלקוח של בעל החברה ברקע (לפי local_id)
            try:
                translate_customer_in_background(
...
def clear_company_results():
    try:
        ...
        # איפוס נתוני החברה בבסיס הנתונים
...
def base_invoice_context(customer_id=None):
    ...
    # שליפת המטבע האקטיבי של המשתמש מתוך העוגיות (ברירת המחדל ללוקאל היא שקל)
    active_currency = request.cookies.get('currency', 'ILS')
    ...
    for c in Customer.query.filter_by(company_id=company_id).all():
        ...
        all_customers_json.append({
            "id": c.id,                     # ✔ מזהה אמיתי
            "local_id": c.local_id,         # ✔ לתרגום בלבד
...
def invoice_context(invoice_id=None):
    try:
        ...
        for c in all_customers:
            ...
            all_customers_json.append({
                "id": c.id,                     # מזהה אמיתי
                "local_id": c.local_id,         # לתרגום / תיקיות
                "customer_name": trans.get("name") or c.customer_name or "",
                "id_number": c.id_number or "",
                "address": trans.get("address") or c.address or "",
                "city": trans.get("city") or c.city or "",
        ...
        base_ctx.update({
            ...
            "invoice_currency": invoice.currency if invoice else "ILS",
        ...
        # שליפת המטבע הפעיל מהעוגיות כגיבוי בטוח בזמן שגיאה
        active_currency_backup = request.cookies.get('currency', 'ILS')
        ...
        return {
            ...
            "invoice_currency": active_currency_backup
        }
...
def save_invoice():
    ...
    if invoice_id:
        ...
        # עדכון המטבע הפעיל מתוך העוגיות במידה והשתנה
        invoice.currency    = request.cookies.get('currency', invoice.currency or 'ILS')
    ...
    # 2. יצירת חשבונית חדשה (מצב יצירה)
        ...
        # שליפת המטבע האקטיבי מהעוגיות של המשתמש (חוק הברזל של ה-30 שפות)
        active_currency = request.cookies.get('currency', 'ILS')
...
def invoice_data():
    ...
    base_currency = getattr(company_obj, "default_currency", "ILS") if company_obj else "ILS"
    ...
    invoices.sort(key=lambda inv: int(str(inv.invoice_number or "0")), reverse=True)
    ...
    for inv in invoices:
        ...
        # שליפת מטבע האמת של החשבונית או נפילה למטבע הבסיס של החברה
        inv.active_currency = getattr(inv, "currency", base_currency) or base_currency
        ...
        if str(inv.customer_id) == "0":
            ...
            customer_i18n_list["0"] = {
                ...
                "id_number": getattr(company_obj, "company_id_number", "") if company_obj else "",
            ...
            if cid not in customer_i18n_list:
                trans = load_customer_translated(inv.customer, language, company_id=company_id) or {}
                customer_i18n_list[cid] = {
                    "name": trans.get("name") or inv.customer.customer_name or "",
                    "address": trans.get("address") or inv.customer.address or "",
                    "city": trans.get("city") or inv.customer.city or "",
                    "postal_code": getattr(inv.customer, "postal_code", "") or "",
                    "id_number": getattr(inv.customer, "id_number", "") or "",
                    "phone": inv.customer.phone or "",
                    "email": inv.customer.email or "",
                    "message": trans.get("message") or inv.customer.message or ""
                }
            trans_name = (customer_i18n_list[cid].get('name', '') or "").lower()
            db_name = (inv.customer.customer_name or "").lower()
        ...
        if search:
            ...
            if not match_search:
                for item in inv.items:
                    if search in str(item.description or "").lower():
                        match_search = True
                        break
        ...
        if match_status and match_search and (not selected_month or inv_month == selected_month) and (not selected_year or inv_year == selected_year):
            if inv.status == "canceled":
                ...
                total_item_cost = sum((item.quantity or 0) * (item.cost_price_at_time or 0) for item in inv.items)
    ...
    for inv in filtered_invoices:
        ...
        inv_currency = getattr(inv, "currency", base_currency) or base_currency
        if inv_currency == base_currency:
            total_amount += float(inv.grand_total or 0.0)
        else:
            try:
                rate = get_exchange_rate(inv_currency, base_currency, inv.invoice_date)
            except Exception as e:
                print(f"⚠️ get_exchange_rate backend fallback fired: {e}")
                rate = 1.0
                
            total_amount += float(inv.grand_total or 0.0) * rate
    ...
    return render_template(
        ...
        total_count=len([inv for inv in filtered_invoices if inv.status != "canceled"]),
        search=search,
    ...
    if request.method == 'POST':
        ...
        # ------------------ CREATE NEW CUSTOMER  ------------------
            ...
            try:
                ...
                print(f"⚠️ Safe customer workspace generation tracking failed: {e}")
...
def api_get_customer(customer_id):
    try:
        ...
        # ------------------ SELF-INVOICE: customer_id = 0 → החברה עצמה ------------------
        if customer_id == 0:
            company_obj = db.session.get(Company, active_company_id)
            company_translated = load_company_translated(company_obj, language) if company_obj else {}
        ...
        # ------------------ לקוח PENDING (User בלי Customer) ------------------
        if not c:
        ...
        # JSON / i18n – אם אין קובץ, חוזר ל-DB
        trans = load_customer_translated(c, language, company_id=active_company_id) or {}
        ...
        print(f"❌ Critical Error inside api_get_customer endpoint pipeline: {e}")
...
def search_customer():
    try:
        ...
        import traceback
        traceback.print_exc()
        print(f"❌ Critical Error inside search_customer route pipeline: {e}")
        return redirect(url_for('customer'))
