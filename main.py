...
def login():
    try:
        ...
        if user.role == 'customer':
            #  חיפוש ראשוני לפי אימייל המשתמש כדי למצוא את השורה המסונכרנת והנכונה
            ...
            if customer:
                ...
                # שכבת הגנה לגיבוי מלא במקרה של רשומות ישנות
...
def register():
    try:
        ...
        if role == 'manager':
            ...
            #  Allow Postgres auto-increment to protect unique constraints
            ...
            # Trigger background company multilingual profile builder
            ...
            try:
                ...
                #  Use background translation helper instead of raw dict file writes
                translate_customer_in_background(
                    customer_id=platform_customer.id,
                    company_id=OWNER_COMPANY_ID,
                ...
                print(f"✔ Manager platform customer workspace created for ID: {platform_customer.id}")
            except Exception as e:
        ...
        # ------------------ 2) CUSTOMER – לקוח של חברה קיימת ------------------
            ...
            if existing_user_in_company:
                if not existing_user_in_company.password_hash or existing_user_in_company.password_hash.strip() == "":
                    ...
                    #  Use true email mapping to discover rows instead of assuming matched user indices
                    customer = Customer.query.filter_by(email=existing_user_in_company.email, company_id=company.id).first()
                    if not customer:
            ...
            #  Reuse the pre-created object directly instead of dropping and overwriting IDs
            if pre_created_customer:
                new_customer = pre_created_customer
                new_customer.role = 'customer'
                new_customer.is_active = True
                if username:
                    new_customer.customer_name = username
                saved_local_id = pre_created_customer.local_id
            else:
                last_c = Customer.query.filter_by(company_id=company.id).order_by(Customer.local_id.desc()).first()
            ...
            try:
                ...
                #  Trigger official multilingual builder engine instead of writing blank text dicts {}
                translate_customer_in_background(
                ...
                print(f"⚠️ Background translation failed during customer registration: {e}")
...
def clients():
    ...
    #  LAYER A: HANDLE COMBOBOX UPDATE (POST)
    if request.method == 'POST':
        try:
            target_id = request.form.get('user_id') # זה ה-ID שמגיע מהשורה בטבלה (מזהה הלקוח/משתמש)
            new_status = request.form.get('status') # 'active' או 'blocked'
            new_role = request.form.get('role')     # 'manager', 'employee', 'customer'

            if target_id and target_id.isdigit():
                tid = int(target_id)
                
                # 1. ננסה למצוא את כרטיס הלקוח מתוך בסיס הנתונים (לפי ה-DB ID הגלובלי שלו)
                cust_row = db.session.get(Customer, tid)
                
                if not cust_row:
                    flash(py_i18n("client.not_found"), "danger")
                    return redirect(url_for('clients'))

                # 2. עדכון הסטטוס בתוך טבלת הלקוחות (קובע האם יופיע בקומבובוקס של החשבוניות!)
                if new_status:
                    cust_row.is_active = (new_status == 'active')
                if new_role:
                    cust_row.role = new_role.lower()

                # 3. בדיקה חכמה: האם הלקוח הזה הוא יוזר ניהולי (מנהל/עובד) או לקוח קצה רגיל?
                if cust_row.email:
                    # מחפשים אם קיים משתמש מערכת עם האימייל המדויק הזה
                    linked_user = User.query.filter_by(email=cust_row.email).first()
                    
                    if linked_user:
                        # הגנת אבטחה: ודא שה-Owner או מנהל החברה רשאים לעדכן
                        if is_owner or str(linked_user.company_id) == str(active_company_id):
                            if new_status:
                                linked_user.is_active = cust_row.is_active
                                linked_user.is_approved = cust_row.is_active
                            if new_role:
                                linked_user.role = new_role.lower()
                        else:
                            flash(py_i18n("auth.no_permission"), "danger")
                            return redirect(url_for('clients'))
                ...
                # 4. הרצת מנוע התרגום ברקע כדי לעדכן את קובץ ה-JSON הפיזי בדיסק של רנדר
                try:
                    translate_customer_in_background(
                        ...
                        city=cust_row.city or "",
                        message=cust_row.message or ""
                    )
                except Exception as file_err:
                    print(f"⚠️ Disk translation sync skipped in clients combo update: {file_err}")

                flash("הנתונים, התפקיד והסטטוס עודכנו בהצלחה בדאטאבייס", "success")
            else:
                flash("לא נמצא משתמש או חברה תואמים לעדכון", "warning")
            ...
            import traceback
            traceback.print_exc()
            print(f"❌ Error updating combobox with Customer/Company logic: {e}")
            flash("שגיאה בעדכון הנתונים", "danger")
            return redirect(url_for('clients'))

    #  LAYER B: RENDER THE CLIENTS VIEW (GET)
    try:
        view_clients = []
        company_obj = None
            ...
            for c in owner_customers:
                ...
                if u:
                    ...
                    if u_role_clean == 'manager' and u.company_id:
                        ...
                        if comp_info:
                            #  Extract all 30 languages generated by your background translation engine
                            comp_translated = load_company_translated(comp_info, language)
                            display_company_name = comp_translated.get("name", comp_info.name)
        ...
        # --- MULTI-TENANT LANGUAGE TRANSLATION PAYLOAD LAYER ---
        if is_owner:
...
def update_access():
    try:
        email    = request.form.get('email', '').strip()
        status   = request.form.get('status', '').strip().lower() # קולט active או blocked מה-HTML     
        ...
        #  העדכון מתבצע על בסיס הערך הנקי מה-HTML (חסין מפני 30 שפות)
        user.is_active   = (status == 'active')
        user.is_approved = (status == 'active')

        #  שליטה מלאה על הזמנים מבלי להרוס את ה-30 יום של הרישום:
        if status == 'blocked':
            # אם חסמת אותו, לא נוגעים בזמן שלו, אבל הוא חסום בגלל ה-is_active = False
            pass 
        else:
            # אם החזרת אותו לפעיל, נשנה את הזמן שלו רק אם בחרת משך זמן ספציפי בתיבה (כמו 15 דקות, שעה וכו')
        ...
        #  Removed id=user.id query lookup to secure database relational layer
        ...
        # סנכרון מול טבלת עובדים (Employees)
        ...
        # שמירת השינויים בבסיס הנתונים בזמן אמת!
        db.session.commit()

        # סנכרון לקבצי דיסק/JSON מקומיים לפי תפקיד
        try:
            if target_user_role_clean == 'employee' and employee_profile:
                ...
                if customer.local_id:
                    ...
                    #   Use background translation engine helper instead of writing empty manual dicts {}
                    translate_customer_in_background(
...
def delete_selected_users():
    try:
        ...
        # Synchronize session state before starting operations
        db.session.expire_all()

        # Check database engine dialect to handle foreign keys dynamically
        ...
        for user_id in ids:
            ...
            # 🚀 INTEGRATED FIX: מוצאים את הלקוח המייצג לפי האימייל שלו, ולא לפי ה-User ID הלא קשור!
            owner_cust_row = None
            if user and user.email:
                owner_cust_row = Customer.query.filter_by(email=user.email, company_id=OWNER_COMPANY_ID).first()
            ...
            # מחיקה פיזית מהדיסק של רנדר כדי שלא יישארו תיקיות זבל
            import shutil
            
            # 1. מחיקת תיקיית הלקוח הרלוונטית תחת חברה 1 (למשל 1_2)
            if owner_local_id:
                ...
                if os.path.exists(cust_folder):
                    try:
                        ...
                        print(f"✔ Physical customer folder deleted from disk: {cust_folder}")
                    except Exception as e:
                        print(f"⚠️ Failed to remove customer folder {cust_folder}: {e}")

            # 2. מחיקת תיקיית החברה המלאה מהדיסק (למשל companies/2)
            if target_comp_id and target_comp_id != OWNER_COMPANY_ID:
                comp_folder = os.path.join(app.config["COMPANY_DIR"], str(target_comp_id))
                if os.path.exists(comp_folder):
                    try:
                        shutil.rmtree(comp_folder)
                        print(f"✔ Physical company folder deleted from disk: {comp_folder}")
                    except Exception as e:
                        print(f"⚠️ Failed to remove company folder {comp_folder}: {e}")

            # מחיקת רשומות מבסיס הנתונים עבור המשתמש והחברה שלו (במידה והוא מנהל שלה)
            if target_comp_id and target_comp_id != OWNER_COMPANY_ID:
                # הסרת רשומות ספציפיות כדי למנוע זיהום של DB
                db.session.execute(db.text('DELETE FROM supplier_purchase WHERE product_id IN (SELECT id FROM product WHERE company_id = :cid)'), {'cid': target_comp_id})
                db.session.execute(db.text('DELETE FROM invoice_item WHERE invoice_id IN (SELECT id FROM invoice WHERE company_id = :cid)'), {'cid': target_comp_id})
                
                Invoice.query.filter_by(company_id=target_comp_id).delete(synchronize_session=False)
                Employee.query.filter_by(company_id=target_comp_id).delete(synchronize_session=False)
                Product.query.filter_by(company_id=target_comp_id).delete(synchronize_session=False)
                Category.query.filter_by(company_id=target_comp_id).delete(synchronize_session=False)
                Supplier.query.filter_by(company_id=target_comp_id).delete(synchronize_session=False)
                Transaction.query.filter_by(company_id=target_comp_id).delete(synchronize_session=False)
                Payment.query.filter_by(company_id=target_comp_id).delete(synchronize_session=False)
                
                # מחיקת החברה עצמה
                comp_obj = db.session.get(Company, target_comp_id)
                if comp_obj:
                    db.session.delete(comp_obj)

            # מחיקת שורת הלקוח המייצג תחת חברה 1 (מנקה סופית את clients.html)
            if owner_cust_row:
                db.session.delete(owner_cust_row)

            # מחיקת המשתמש עצמו מטבלת Users
            if user:
        ...
        print(f"✔ Purge Route Success: {deleted_count} records cleanly deleted from system")
        flash(py_i18n("client.delete_success"), "success")
        return redirect(url_for('clients'))
        ...
        import traceback
        traceback.print_exc()
        flash("שגיאה חמורה במחיקת המשתמשים הנבחרים", "danger")
        return redirect(url_for('clients'))
...
def update_role():
    try:
        ...
        #  Removed db.session.expire_all() to protect data persistence layer syncing
        ...
        if old_role != new_role:
            ...
            if new_role == 'manager':
                ...
                # Trigger background company profile generation engine
                translate_company_in_background(new_company.id, new_company.name, "", "", "", "", "", user.email, "")
                ...
                try:
                    ...
                    #  Use translation engine instead of writing blank dictionary keys {}
                    translate_customer_in_background(
...
def base_invoice_context(customer_id=None):
    ...
    if customer_id:
        if str(customer_id) == "0":
            if company_obj:
                #  Tightly bind all self-invoice address keys to ensure correct fallback layouts
                customer_data = {
                    "id": "0",  
                    "local_id": 0,
            ...
            if c_obj:
                ...
                if isinstance(customer_data, dict):
                    ...
                    customer_data["local_id"] = c_obj.local_id
    ...
    try:
        ...
        if invoice_id:
            ...
            if invoice and invoice.company_id != company_id:
                print(f"🔒 Security Alert: User {current_user.id} tried to access unauthorized invoice {invoice_id}")
                invoice = None
        ...
        #  איחול שורות הפריטים הקיימים בחשבונית והזרקת המק"ט הנכון לכל שורה בנפרד!
        # פותר את בעיית כפל המק"טים ומאפשר ל-loadExistingItems לעבוד  ללא תקלות
        items_json = []
        if invoice:
            items = InvoiceItem.query.filter_by(invoice_id=invoice.id).all()
            for item in items:
                # זיהוי המוצר ב-SQL לפי ה-local_id השמור בשדה product_id של שורת החשבונית
                target_p = None
                if str(item.product_id).isdigit():
                    target_p = Product.query.filter_by(local_id=int(item.product_id), company_id=company_id).first()

                # חילוץ המק"ט (SKU) והשם האמיתיים המתורגמים מתוך קובץ האב בדיסק במידה והמוצר קיים
                ...
                #  Suppress SKU rendering entirely if this is an internal self-invoice transaction
                final_sku = "" if is_self_invoice else str(row_sku).strip()
        ...
        #  Pre-calculate total units sold for all items of this company 
        # using exactly 1 single grouped database query instead of hammering SQL inside the product loop!
        sold_aggregation = (
            db.session.query(InvoiceItem.product_id, func.sum(InvoiceItem.quantity))
            .join(Invoice)
            .filter(
                InvoiceItem.income_category == 'product',
                Invoice.company_id == company_id,
        ...
        total_sold_map = {str(pid): int(qty or 0) for pid, qty in sold_aggregation}

        #  בניית רשימת המוצרים לבחירה (Products Combobox)
        # שואב את מילון 30 השפות הרשמי ומחזיר מחרוזת שטוחה נקייה בשפה הנוכחית 
        products_json = []
        my_products = Product.query.filter_by(company_id=company_id).all()
        for p in my_products:
            p_key = str(p.local_id if p.local_id is not None else p.id)
            ...
            #  Pull quantities straight out of memory instead of hitting the database
            total_sold = total_sold_map.get(p_key, 0)
            ...
            #  Suppress SKU inside searchable selection box arrays if running self-invoice context
            p_sku = p.sku if p.sku else item_file.get("sku", str(p.local_id if p.local_id is not None else p.id))
            final_p_sku = "" if is_self_invoice else str(p_sku).strip()
...
@app.route('/company/<int:company_id>/invoice/save', methods=['POST'])
@login_required
def save_invoice(company_id):
    # וידוי הרשאות גישה לחברה הפעילה
    company = Company.query.filter_by(id=company_id, user_id=current_user.id).first()
    if not company:
        flash('חברה לא נמצאה או שאין לך הרשאה מתאימה', 'danger')
    ...
    customer_name_val = request.form.get('customer_name', '').strip()
    customer_phone_val = request.form.get('customer_phone', '').strip()
    customer_email_val = request.form.get('customer_email', '').strip()

    db_customer_id = None

    if customer_id_val and customer_id_val.strip():
        cust = Customer.query.filter_by(id=customer_id_val, company_id=company_id).first()
        if not cust:
            cust = Customer.query.filter_by(local_id=customer_id_val, company_id=company_id).first()
        if cust:
            db_customer_id = cust.local_id if cust.local_id is not None else cust.id
            if customer_phone_val:
                cust.phone = customer_phone_val
            if customer_email_val:
                cust.email = customer_email_val
            db.session.flush()

    if not db_customer_id and customer_name_val:
        max_local_cust = db.session.query(db.func.max(Customer.local_id)).filter(Customer.company_id == company_id).scalar()
        next_cust_local_id = (max_local_cust or 0) + 1

        new_cust = Customer(
            company_id=company_id,
            local_id=next_cust_local_id,
            name=customer_name_val,
            phone=customer_phone_val,
            email=customer_email_val
        )
        db.session.add(new_cust)
        db.session.flush()
        db_customer_id = new_cust.local_id

    # סיכומי כספים וחישובים כלליים מהטופס
    ...
    vat_rate = clean_float(request.form.get('vat_rate', 17))
    ...
    if invoice_id:
        ...
        if not invoice:
            flash('החשבונית לעריכה לא נמצאה', 'danger')
            return redirect(url_for('invoices_list', company_id=company_id))

        # שלב א': שחזור מלאי מקורי (הפוך) לפני מחיקת הפריטים הישנים
        old_items = InvoiceItem.query.filter_by(invoice_id=invoice.id).all()
        for old_item in old_items:
            prod = Product.query.filter_by(id=old_item.product_id, company_id=company_id).first()
            if not prod:
        ...
        existing_trans = Transaction.query.filter_by(invoice_id=invoice.id, company_id=company_id).first()
        if existing_trans:
    ...
    # ====== 2. יצירת חשבונית חדשה לחלוטין (מצב יצירה) ======
        ...
        # חישוב ה-local_id הבא עבור התנועה הפיננסית - מבודד רק על החברה הפעילה
        max_local_trans = db.session.query(db.func.max(Transaction.local_id)).filter(Transaction.company_id == company_id).scalar()
        next_trans_local_id = (max_local_trans or 0) + 1
...
def invoice_data():
    ...
    for inv in invoices:
        ...
        if not search:
            ...
            # הרחבת החיפוש לסריקת תיאורים פנימיים בלבד (המק"טים מוסתרים לחלוטין)
            if not match_search:
                for item in inv.items:
                    item_desc = str(item.description or "").lower()
                    if search in item_desc:
                        match_search = True
                        break
...
def api_get_customer(customer_id):
    try:
        ...
        if customer_id == 0 or customer_id == active_company_id:
            company_obj = db.session.get(Company, active_company_id)
            company_translated = load_company_translated(company_obj, language) if company_obj else {}
...
def profit():
    try:
        ...
        #  Replaced N+1 dirty database lookups with the pre-cached memory mapping index dictionary
        item_names_map = {}
        for customer in all_customers:
            for inv in customer.invoices:
                if inv.status == "canceled":
