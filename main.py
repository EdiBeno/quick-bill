...
@app.route('/clients', methods=['GET', 'POST']) # <-- FIXED: Now accepts POST requests for combobox updates
@login_required
def clients():
    language = get_lang()
    ...
    if request.method == 'POST':
        try:
            ...
            if target_id and target_id.isdigit():
                ...
                # 1. ננסה למצוא את כרטיס הלקוח בתוך חברה 1 כדי לשלוף את האימייל האמיתי של החברה
                cust_row = Customer.query.filter_by(id=tid).first()
                
                user_to_update = None
                if cust_row and cust_row.email:
                    # אם מצאנו לקוח עם אימייל, נחפש את ה-User הניהולי שמתאים לאימייל הזה
                    user_to_update = User.query.filter_by(email=cust_row.email).first()
                
                # אם לא מצאנו לפי אימייל, ננסה להביא את ה-User ישירות לפי ה-ID של השורה
                if not user_to_update:
                    user_to_update = db.session.get(User, tid)

                if user_to_update:
                    # הגנת אבטחה: ודא שה-Owner או מנהל החברה רשאים לעדכן
                    if is_owner or str(user_to_update.company_id) == str(active_company_id):
                        
                        # א. עדכון מצב חסימה/פעיל (עמודה 9)
                        if new_status:
                            user_to_update.is_active = (new_status == 'active')
                        
                        # ב. עדכון תפקיד (עמודה 10) - מנהל/עובד/לקוח
                        if new_role:
                            user_to_update.role = new_role.lower()
    ...
    try:
        ...
        # --- MULTI-TENANT FIXED LANGUAGE TRANSLATION PAYLOAD LAYER ---
        if is_owner:
        ...
        for emp in all_employees_db:
            try:
                # Force runtime variable language rather than cookie fallback context
        ...
        for cust in all_customers_db:
            try:
                # Force dynamic language parameter explicitly inside runtime context
...
def update_access():
    try:
        ...
        #  עדכון הסטטוס על בסיס ה-value מה-HTML
        ...
        if employee_profile and hasattr(employee_profile, 'is_active'):
            employee_profile.is_active = user.is_active

        #  שמירה חזקה וסופית ל-Postgres!
        db.session.commit()

        try:
            if target_user_role_clean == 'employee' and employee_profile:
                target_folder, json_file_path, _ = get_company_clock_paths(search_company_id, employee_profile.id, current_year, current_month)
                ...
                translate_customer_in_background(customer_id=customer.id, company_id=search_company_id, name=customer.customer_name, address=cust_address, city=cust_city, message=cust_message)
...
def delete_selected_users():
    try:
        ...
        for user_id in ids:
            ...
            # --- STEP 1: Delete User Rows Safely (Enforcing Clean Foreign Key Hierarchy for Postgres) ---
            try:
                # Raw text queries use generic bindings for safety across dialects
                db.session.execute(db.text("DELETE FROM shift_states WHERE employee_id = :uid"), {"uid": target_uid})
                db.session.execute(db.text("DELETE FROM employee WHERE id = :uid"), {"uid": target_uid})
                db.session.execute(db.text("DELETE FROM password_reset_tokens WHERE user_id = :uid"), {"uid": target_uid})
                db.session.execute(db.text("DELETE FROM customer WHERE id = :uid"), {"uid": target_uid})
                db.session.execute(db.text("DELETE FROM users WHERE id = :uid"), {"uid": target_uid})
                db.session.flush()
            except Exception as e:
                print(f"User rows query bypass check error: {e}")

            # --- STEP 2: Delete Entire Company Substructure if valid ---
            if target_comp_id and target_comp_id != OWNER_COMPANY_ID:
                try:
                    # Explicit hierarchy to satisfy PostgreSQL constraints
                    ...
                    db.session.execute(db.text("DELETE FROM transactions WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM payments WHERE company_id = :cid"), {"cid": target_comp_id})
                    ...
                    db.session.execute(db.text("DELETE FROM invoice WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM product WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM category WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM supplier WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM shift_states WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM employee WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM users WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM customer WHERE company_id = :cid"), {"cid": target_comp_id})
                    db.session.execute(db.text("DELETE FROM company WHERE id = :cid"), {"cid": target_comp_id})
                    db.session.flush()
                except Exception as e:
                    print(f"Company row bypass check error: {e}")

                # --- STEP 3: Clean up Server Physical Directories ---
                try:
                    import os
                    import shutil
                    ...
                    print(f"Failed to remove company directory: {e}")
                ...
                try:
                    ...
                    print(f"Failed to remove employee directory: {e}")
        ...
        # Re-enable foreign keys if we are operating on SQLite environment
        if is_sqlite:
...
def update_role():
    try:
        ...
        if not owner_mode:
            if target_user_role_clean in ['manager', 'owner'] or new_role in ['manager', 'owner']:
                flash(py_i18n("auth.no_permission"), "danger")
                return redirect(url_for('clients'))
            if user.company_id != active_company_id:
                flash(py_i18n("auth.no_permission"), "danger")
        ...
        if old_role != new_role:
            ...
            if new_role == 'manager':
                ...
                for o_c in old_cust_records: db.session.delete(o_c)
                old_emp_records = Employee.query.filter_by(email=user.email, company_id=old_company_id).all()
                for o_e in old_emp_records: db.session.delete(o_e)
                ...
                owner_customer = Customer(
                    id=user.id, local_id=next_local_id, company_id=OWNER_COMPANY_ID,
                    customer_name=company_name, email=user.email, role='customer',
                    is_active=user.is_active, date=datetime.today().strftime('%d/%m/%Y')
                )
                ...
                try:
                    folder = os.path.join(app.config["CUSTOMERS_DIR"], f"{OWNER_COMPANY_ID}_{next_local_id}")
                    os.makedirs(folder, exist_ok=True)
                    with open(os.path.join(folder, "customer.json"), "w", encoding="utf-8") as f:
                        json.dump({"name": {}, "address": {}, "city": {}, "message": {}}, f, ensure_ascii=False, indent=4)
                except Exception as e:
                    print(f"⚠️ Failed to create manager customer workspace: {e}")

            elif new_role == 'customer':
                if owner_mode and company_id: user.company_id = int(company_id)
                ...
                if emp_prof:
                    ...
                    for o_s in old_shifts: db.session.delete(o_s)
                ...
                if not customer:
                    ...
                    next_local_id = 1 if not last_c else (last_c.local_id or 0) + 1
                    customer = Customer(
                        id=user.id, local_id=next_local_id, customer_name=user.username or user.email or "לקוח חדש",
                        email=user.email, company_id=user.company_id, role='customer',
                        is_active=user.is_active, date=datetime.today().strftime('%d/%m/%Y')
                    )
                    db.session.add(customer)
                else:
                    ...
                    customer.is_active = user.is_active
                    if not customer.customer_name: customer.customer_name = user.username or user.email
                db.session.flush()
                ...
                if not employee_profile:
                    employee_profile = Employee(
                        company_id=user.company_id, employee_name=user.username or user.email or "עובד חדש",
                        email=user.email, id_number="", user_id=user.id, is_active=user.is_active
                    )
                    db.session.add(employee_profile)
                    db.session.flush() 

                try:
                    target_folder, json_file_path, _ = get_company_clock_paths(user.company_id, employee_profile.id, current_year, current_month)
                    os.makedirs(target_folder, exist_ok=True)
                    json_payload = {
                        "hours_table": {"work_day_entries": [], "tax": {}},
                        "is_active": user.is_active, "email": user.email, "employee_name": employee_profile.employee_name
                    }
                    with open(json_file_path, "w", encoding="utf-8") as f:
