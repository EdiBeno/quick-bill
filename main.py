...
def save_invoice():
    ...
    if str(customer_id).strip() == "0":
        ...
        # רשת ביטחון: אם לקוח ההנהלה לחשבונית עצמית לא קיים ב-DB - נברא אותו בשנייה זו
        if not owner_customer:
            try:
                owner_customer = Customer(
                    name="חשבונית עצמית - הנהלה",
                    email=current_user.email,
                    company_id=OWNER_COMPANY_ID,
                    phone="000",
                    city="Internal"
                ...
                db.session.rollback()
                owner_customer = Customer.query.filter_by(company_id=OWNER_COMPANY_ID).first()
    ...
    if invoice_id:
        ...
        # שלב ב': עדכון נתוני כותרת החשבונית ( use db_customer_id)
        ...
        # שלב ה': עדכון או יצירת תנועה פיננסית ביומן התנועות (Transaction) -  use db_customer_id
