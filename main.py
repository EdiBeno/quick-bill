...
def format_vat(vat):
    if not vat:
        return ""
    vat = ''.join(filter(str.isdigit, vat))
    # Common VAT formatting: group digits for readability
    return " ".join(vat[i:i+3] for i in range(0, len(vat), 3))
            ...
            # CUSTOMER JSON SYNC 
                ...
                if customer.local_id:
                    ...
                    if folder:
                        ...
                        # אם הקובץ כבר קיים, טוענים את כל התרגומים הקיימים של 30 השפות מבלי לדרוס אותם!
                        if is_file_exists:
                            try:
                        ...
                        # הבטחת מבנה השדות המתורגמים לדיסק
                        ...
                        # מעדכנים רק שדות מטא-דאטה לא מתורגמים במידת הצורך
                        existing_data["is_active"] = user.is_active
                        
                        # שמירה מיידית של המידע ללא שום הפעלה של טרדים מיותרים
                        with open(file_path, "w", encoding="utf-8") as f:
                            json.dump(existing_data, f, ensure_ascii=False, indent=4)
                        
                            print(f"✔ Airtight Customer data updated in-place via existing JSON. No threads spawned.")
        ...
        if old_role != new_role:
            ...
            # CUSTOMER
                ...
                try:
                    ...
                    # הוסר לחלוטין: מניעת קריאות הרקע המפוזרות של התרגום בזמן עדכון התפקיד!
                    # המערכת תקרא את הנתונים ישירות דרך load methods מהדיסק ללא עומס וטרדים
...
def customer():
    ...
    if request.method == 'POST':
        ...
        # 🔥 הגנת זהב עבור Self Invoice (חשבונית עצמית)
        ...
        # ------------------ CREATE NEW CUSTOMER ------------------
            ...
            # 🛠️ תיקון הזהב: המזהה id=user_obj.id הוחזר למקומו כדי לאפשר שמירה תקינה בבסיס הנתונים
            new_customer = Customer(
                id=user_obj.id,
                company_id=active_company_id, 
                local_id=next_local_id,  
